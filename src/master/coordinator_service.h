...
namespace mini_borg
{
    ...
    // Creates class that inherits the empty functions from generated Service class from gRPC proto, final means no further inheritance
    class CoordinatorServiceImpl final : public Coordinator::Service
    {
    public:
        CoordinatorServiceImpl(const std::string &db_conn_str);
        ...
        grpc::Status FinishJob(grpc::ServerContext *context, const mini_borg::FinishJobRequest *request, mini_borg::FinishJobResponse *response);

    private:
        void SaveJobToDb(const std::string &id, const std::string &name, const Resource &res, const std::string &worker_id);
